...
static int part_1_2 (char **line, t_data *data,int flag, int count)
{
    char *tmp;
    // t_data *temp;
    // t_data *temp_2;
	...
	if (!flag)
	{
        ...
        *line = tmp;
        if (count == (int)ft_strlen(data->buff_all))
            count--;
        ft_memmove(&data->buff_all[0],&data->buff_all[count+1],ft_strlen(data->buff_all)-count-1);
		ft_bzero(&data->buff_all[ft_strlen(data->buff_all)-count-1], count+1);
		return(1);
	}
...
static int part_1_1 (char **line, t_data *data)
{
    ...
    if (!(data->ret = read (data->fd, buff_all, BUFF_SIZE)))
    {
        if (!(data->buff_all) || data->buff_all[0] == '\0')
            ...
            return(part_1_2 (line, data,0,ft_strlen(data->buff_all)));
    }
    if (data->ret < 0)
        return (-1);
    buff_all[data->ret] = '\0';
...
int get_next_line(const int fd, char **line)
{
	...
	while (fd > -1) 
	{
		...
		if (data->count >= 0)
			if ((data->retur = part_1_2(line,data,0,data->count)) < 2)
				return(data->retur);
		if (data->count < 0) 
